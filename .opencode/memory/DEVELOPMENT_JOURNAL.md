...
CFG-001 then added one service facade for initialize, reload, rollback, provider calls, MCP calls, status, and close. Provider and MCP resources use the same generation service without exposing secrets in status. Focused generation/service tests passed `20/20`; complete core `630/630`, closure `175`, scanner clean, package `2/2`, and runtime probe `20,690` passed. Restart metadata persistence remains.

CFG-001 then persisted only generation counters, current ID, and revision. Restart creates a fresh monotonic generation; response-loss writes reconcile by read-back; stale write failures close the service. Focused state/service tests passed `14/14`; complete core `633/633`, closure `175`, scanner clean, package `2/2`, and runtime probe `20,691` passed. Production Pi provider wiring is blocked because before-request and after-response hooks do not share a request correlation ID, so an in-flight lease cannot be released reliably. The item is marked blocked rather than adding a false binding.

The missing Pi lifecycle correlation contract was tracked at `https://github.com/earendil-works/pi/issues/8380`. The clean selective fork candidate `coco-v0.82.1-coco.6` added correlation to headers as well as payload/response/terminal events. CoCo now loads `coco-provider-generation.mjs` as a native extension; it hashes assembled header values in memory, acquires a generation lease, aborts on missing/duplicate correlation, and releases on every terminal outcome. Candidate run `32355915832` passed; the CoCo frozen batch passed core `637/637`, closure `172`, scanner clean, package `2/2`, and runtime probe `21,354`. CFG-001 is complete within the pinned-candidate boundary.
...
EVID-001 is now complete at the final provider-request seam: the provider lifecycle extension emits generation/system/message/tool facts, reconstructs the projection, and verifies it against the ledger before the request continues. Corrupt, symlink, missing, reordered, duplicate, and cross-request records fail closed. Final core passed `645/645`, closure `172`, scanner clean, package contract, and runtime probe `21,356`.
...
EVID-002 initial fence reuses REC-001's journal and now wraps MCP tool effects: persist intent before call, record result before returning, replay durable results, and never replay uncertain effects. Focused fence/MCP tests pass; complete core `647/647`, closure `172`, scanner clean, package contract, and runtime probe `21,357` pass. Provider/Bash fence rollout remains.

EVID-002 then integrated the same split lifecycle into provider and Bash hooks. Provider intent is durable before network execution, response acknowledgement is persisted before stream content consumption, errors/aborts become uncertain, Bash intent is durable before tool execution, and unpersisted Bash results are replaced with an explicit uncertain result. Complete core passed `650/650`, closure `172`, scanner clean, package contract, and runtime probe `21,358`. EVID-002 is complete.
...
The version surface was synchronized to 0.7.0. Final gates passed: release/version focused `35/35`, core `676/676`, integrity `39/39` with one host-timeout case rerun on identical bytes, closure `172`, scanner clean, package `2/2`, runtime probe `21,365`, Node 22 performance, and real offline tar byte equality/install/version/uninstall. npm remains unpublished.

The initial TOOL-001 `coco.8` candidate was rejected after archive inspection showed its nested core still contained the old whole-batch serialization logic. Candidates `coco.9` and `coco.10` closed archive and manifest binding gaps; final `coco-v0.82.1-coco.11` also binds the staged shrinkwrap to the matching immutable core asset. Workflow `32685150665`, archive inspection, CoCo install, and isolated CoCo lock reconstruction all contain the segmented barrier implementation. Final gates pass core `676/676`, integrity `39/39`, closure `172`, scanner clean, package `2/2`, and runtime probe `21,368`.

The published `v0.7.1` was cut from merged TOOL-001 main commit `aa7fd596bc8539a7e57c0c5872e27915c0befcd3` and tag `v0.7.1`. Release workflow `32731763396` passed all four permission-isolated stages and published nine assets at https://github.com/bit-cook/coco/releases/tag/v0.7.1. Its final draft flag was corrected by PR #23 after verification exposed that the workflow left the verified draft private; no asset rebuild or unverified publication was used. `verify-main` moved from the offline `coco-ci-local` runner to pinned Node `22.19.0` on `ubuntu-24.04`; post-release main CI `32738293959` passed. Version/release focused contracts pass `68/68`, typechecks pass, core `676/676`, integrity `39/39`, closure `172`, scanner clean, package `2/2`, runtime probe `21,368`, and Node `22.19.0` startup performance pass. npm remains intentionally unpublished. No new research item is active.

Post-release public verification downloaded the `v0.7.1` nine-asset release and validated every file listed in `SHA256SUMS`. Public `install.sh` installed into an isolated home and launcher directory, `coco --version` returned `0.7.1`, and public `uninstall.sh` removed the isolated install, agent state, and launcher. The host's pre-existing `/usr/local/bin/coco` was only reported and was not modified. npm still returns `E404` for `coco@0.7.1`.
...
## 2026-08-27 COWEB-002 Native Co Web Implementation

COWEB-002 replaces the runtime `@lyhue1991/pi-web@latest` install and Next.js server with a CoCo-native loopback service. The pinned Pi Web 0.8.17 browser client is vendored as package data under `coweb/` (`scripts/vendor-coweb-static.mjs` verifies the pinned version, source revision, and sha512 lockfile, stages atomically, and writes `coweb/desktop/SNAPSHOT.json` + MIT attribution). `scripts/coweb-native-service.mjs` owns all API/session/model/SSE/file/auth behavior using the bundled `@earendil-works/pi-coding-agent` session services; `scripts/coweb.mjs` spawns it detached with a `COWEB_READY` handshake and rejects `--update`/`--hostname`/`--allow-host`; `scripts/coweb-proxy.mjs` preserves public `coco:<password>` basic auth and requires `--public-host` with `--password`. Desktop CSS is untouched; mobile overrides live only in `coweb/coweb-mobile.css` at `max-width: 640px`. Hardening: loopback host/origin validation (403 `COWEB_UNTRUSTED_REQUEST`), session-bound realpath-contained file preview, workspace set via `/api/cwd/validate`, session model restore, and `SettingsManager.create(..., { projectTrusted: false })`.

Verification on the uncommitted tree (`be2d1e8` + COWEB-002 batch): focused gates `node --test test/coweb-command.test.mjs test/coweb-proxy.test.mjs test/coweb-native-service.test.mjs` 7/7 pass; `npm run typecheck:coweb` pass; `git diff --check` clean. Real prompt/SSE E2E on isolated loopback port 30145 reached `connected` → agent events → `prompt_done`/`agent_end` with reply text. Chrome headless CDP confirmed 1440px desktop unchanged, 390/640px no horizontal overflow, 641px desktop intact. `scripts/package-asset-map.v1.json` (1473 entries) and `resources/runtime-integrity-manifest.v1.json` (21665 entries) + `.sha256` were regenerated after the source freeze; `NOTICE`, `package.json` files list, and EN/ZH `coco-cli.md` updated. Awaiting explicit commit authorization; no commit, push, tag, or publish performed.

## 2026-08-27 COWEB-002 Asset Regeneration Checkpoint and Native Launch

First `bin/coco coweb` launch exposed a generated-asset defect: the vendored snapshot shipped the compiled test-route chunk `_next/static/chunks/app/api/models-config/test/route-*.js`. The package asset map (correctly) listed it, `generate-runtime-integrity-manifest.mjs` unions asset-map paths into the manifest, but `coco-bootstrap.cjs` `structureCheck` excludes every `test` path component, so the rebuilt runtime snapshot failed with `RUNTIME_INTEGRITY_REVALIDATION_FAILED`. Fix: `scripts/vendor-coweb-static.mjs` now prunes compiled `test` route-chunk directories under `_next/static/chunks/app/api` before computing `SNAPSHOT.json` (server-side chunks never loaded by the static client; the native service owns `/api/*`). Generation record — sources: `scripts/vendor-coweb-static.mjs`; command: `node scripts/vendor-coweb-static.mjs` (pruned 1 path, treeSha256 `decc522a…`), then `node scripts/generate-asset-map.mjs`, `node scripts/generate-runtime-integrity-manifest.mjs`; outputs changed: `coweb/**` re-staged, `scripts/package-asset-map.v1.json`, `resources/runtime-integrity-manifest.v1.json` + `.sha256`. Gates after generation: focused coweb tests 7/7, `npm run typecheck:coweb` pass, `git diff --check` clean, `node scripts/runtime-integrity.mjs` pass. Prior integrity evidence superseded.

Operational launch (authorized by user request to start the latest coweb with a public address): stopped the legacy pi-web instance (30141), its proxy (30142), and the 30145 E2E instance. Started native stack via `node bin/coco coweb --port 30141 --password <generated> --public-host 4a030f4d4bc8f8.lhr.life` (pid 2244616; loopback 30141 + authenticated proxy 30142; password at `/root/.coco/coweb-public-password.txt`, mode 600). Public transport is a raw SSH reverse tunnel to localhost.run (`ssh -R80:127.0.0.1:30142 nokey@localhost.run`); note the first assigned subdomain was revoked after upstream-refused cycles and re-assigned to `4a030f4d4bc8f8.lhr.life`, so the service was relaunched against the live host. Public verification: no-auth `/` → 401; authenticated `/` → desktop client HTML with mobile viewport + `coweb-mobile.css`; `/api/models` → native model list with default `agnes/agnes-2.5-flash`. Anonymous localhost.run domains are ephemeral; the address changes if the tunnel reconnects.

## 2026-08-27 CONTROL-001 Control Task Pipeline End-to-End Repair

User-reported: public control dashboard loads but tasks flash endlessly and approve produces no result. Real E2E against the live service reproduced instantly — every task transitioned queued→failed in ~1s with `lastError = "coco: ERR_MODULE_NOT_FOUND"`. Root cause chain: detached supervisor derives the agent entrypoint as `resolve(root, "bin", "coco")` where `root` is the bootstrap CAS runtime snapshot, but `bin/coco` is a trust anchor deliberately absent from snapshots — so the inner agent spawn never existed and no task could ever run. Secondary UI defects masked it: the 3s poller wiped and fully rebuilt the task grid serially per task (visible flashing), action errors were swallowed silently, and one rejection truncated rendering.

Fixes (lease `opencode-control-001`, base `051b319`, scope `scripts/coco-bootstrap.cjs`, `scripts/task-run-supervisor-main.mjs`, `control/public/*`, `test/task-run-supervisor.test.mjs`): bootstrap now exports the verified app root as `COCO_APP_ROOT` (resolved before snapshot staging) and stamps `appRoot` into `.runtime-complete.json`; supervisor resolves the launch root from env → marker → local fallback, healing even orphaned pre-upgrade runners; frontend rebuilt around keyed incremental DOM updates (parallel detail fetches, node identity preserved across polls, connection-loss surfacing after two failed polls, inline error notices, button pending states). Regression test `supervisor launches the app-root coco entry when COCO_APP_ROOT is set` added; governed assets regenerated post-freeze.

Verification, all on the uncommitted tree: focused suites `task-run-supervisor` 15/15, `control-service`+`task-control-state` 49/49; real API lifecycle — approved task queued→running→completed with non-empty agent result (worktree commit `8a7d92a` produced by the run), blocked task then `/approve`→completed, cancel→`TERMINATED_BY_USER`, `/stop-all` terminated stale runner, events/logs/receipt endpoints return data; headless-browser session over the PUBLIC URL: connect-by-token, form task creation, approve click, completion text rendered, zero grid churn across steady-state polls (7/7 assertions). Operational notes: stale runner daemons pin old snapshot dirs via state files (`resolveRuntimeRoot`) — after manifest-changing upgrades a runner/control restart plus `/v1/tasks/stop-all` reset is required; localhost.run anonymous domains are ephemeral and rotated to `e87c8767d21f36.lhr.life`. Commit not yet authorized at checkpoint time.

## 2026-08-27 CONTROL-001 Addendum: cwd validation at creation, prominent failure rendering, CAS-drift-proof runtime resolution

User's "approve has no reaction" was a masked failure: their cwd did not exist, runner provisioning rejected TASK_CWD_INVALID and reset to blocked (~1s), indistinguishable visually from a dead button. Fixed at three layers: HTTP create-path validation (immediate 400 family), resolveRuntimeRoot preferring env CAS root only after marker.manifestHash==sha256(bundled manifest) self-check then state pins (kills pre-upgrade snapshot reuse that had also silently blocked earlier hot-fix rollout — service now provably serves newest key 96cccfe1…), and card-level error styling. Public headless-browser regression set all-pass; API 400 matrix verified; happy path re-completed through public URL. Journal supersedes prior evidence for touched files; commit still pending authorization.

## 2026-08-27 WEB-001 Command Rename to `coco web` and CoCo Web Branding

Per user request the command surface moved from `coco coweb` to `coco web`, keeping `coweb` as a runtime alias (dispatcher accepts both; help text documents the alias; installed shims now write both `$COCO_BIN_DIR/web` and legacy `$COCO_BIN_DIR/coweb` with install/uninstall backup-and-cleanup parity; uninstall matcher accepts both redirect spellings). The frozen Pi Web client is branded at vendor time in `vendor-coweb-static.mjs` before tree hashing: "Co Web"/"CoWeb" → "CoCo Web" across index.html/index.rsc/offline.html/manifest.webmanifest and sw.js CACHE_PREFIX → "coco-web"; launcher success line now reads "native CoCo Web". Attribution untouched by design: SNAPSHOT.json keeps source `@lyhue1991/pi-web` + MIT license/integrity per attribution requirement, and the functional `pi-theme` localStorage key remains (visible-behavior risk without benefit). Contract surfaces updated together: installer shim assertions (web primary + coweb retained) and DEVELOPMENT_PLAN next-target line now "native CoCo Web (`coco web`) distribution" with matching contract regex.

Evidence: vendor re-run treeSha256 `e94d14c4…`; gates — coweb trio + installer + plan-contract + control-service + task-run-supervisor 71 tests all green after one contract-regex backtick fix, typecheck pass, runtime integrity pass on regenerated manifest, git diff --check clean. Live: `bin/coco web` launches (pid fresh on 30141/30142), real `bin/coco coweb` alias invocation verified then cleaned; public tunnel regenerated → https://03c9ce883aa797.lhr.life returns 401 unauthenticated and serves `<title>CoCo Web</title>` with credentials; headless browser (390px) confirms mobile css link, no horizontal overflow, `/api/models` 200 with default agnes model. Screenshot `/tmp/cocoweb-branded.png`. Commit pending authorization.

## 2026-08-27 v0.8.0 Release Preparation

Release candidate bundles CONTROL-001 (control task pipeline repair), WEB-001 (`coco web` rename + CoCo Web vendor branding), and the 0.8.0 version bump across package/product identity/vscode/contract tests/docs/site/install URLs. Truth blocks move to branch `release/v0.8.0` with released baseline v0.7.4@096f830; target = publish v0.8.0 via the tag-triggered private-draft pipeline (9 assets, receipt-verified). npm remains intentionally unpublished.

## 2026-08-27 v0.8.0 Released

Release published. Release commit `bea556c` tagged `v0.8.0` on branch `release/v0.8.0` (main fast-forwarded `eb699b6..3e0840f`). GitHub Release run 33092046010 succeeded and published 9 assets (offline linux-x64 bundle 97.3MB + sha256, tgz 40MB + sha256, vscode vsix + sha256, install.sh, uninstall.sh, SHA256SUMS) at 2026-08-27T17:41:48Z; CI on main 33092048618 success. Pages run 33092048696 failed at deploy-pages; retried via site date refresh commit `3e0840f` → run 33100031034 success. Live verification: release install.sh downloads 200; site (served through custom domain osome.work/coco via Pages redirect) shows CoCo 0.8.0 homepage, v0.8.0 uninstall command, and plan baseline v0.8.0. Pre-tag full suite: 726/726 (37.8 min, TMPDIR on persistent disk after a /tmp tmpfs ENOSPC wave produced 154 collateral failures that were disproven by cleanup and re-run), plus typecheck, identity check, runtime integrity, tarball closure approved (172 files), secret scan clean. One real regression found and fixed during the sweep: roadmap contract and launcher-fixture copy filters pinned 0.7.4 / excluded the Next `app/api/agent` route via an `/agent/` substring rule (now top-level-only), which had broken launcher-preflight fixtures. Release notes body remains the automated ownership marker; npm intentionally unpublished.
