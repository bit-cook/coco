...
export function createModelPanelController({ refreshTimeoutMs = 15_000, render = renderModelPanel, runtime } = {}) {
  ...
  return Object.freeze({
    ...
    async open({ query, signal } = {}) {
      ...
      const operation = controller(signal); active.add(operation); let timedOut = false; const timeout = setTimeout(() => { timedOut = true; operation.controller.abort(); if (!normalized && !closed) publish({ ...state, refresh: { errors: Object.freeze([]), status: "aborted" }, status: "ready" }); }, refreshTimeoutMs);
      if (!normalized) {
      ...
      catch (error) { if (!cancelled(error)) throw error; if (!closed) publish({ ...state, refresh: { errors: Object.freeze([]), status: "aborted" }, status: "ready" }); return timedOut ? { kind: "panel" } : { kind: "cancelled" }; }
