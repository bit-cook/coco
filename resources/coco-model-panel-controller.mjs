...
export function createModelPanelController({ refreshTimeoutMs = 15_000, render = renderModelPanel, runtime } = {}) {
  ...
  let state = Object.freeze({ panel: null, query: undefined, refresh: Object.freeze({ errors: Object.freeze([]), reason: null, status: "idle" }), scope: null, status: "idle" });
  let closed = false, opened = false, selecting = false, snapshot = null;
  ...
  function controller(signal) { const value = new AbortController(); const abort = () => value.abort(); if (signal?.aborted) value.abort(); else if (signal) signal.addEventListener("abort", abort, { once: true }); return { abort, controller: value, signal }; }
  ...
  async function runSelection(model, signal, updateState = false) {
    if (selecting) throw new Error("MODEL_PANEL_ACTION_IN_PROGRESS"); selecting = true; const operation = controller(signal); active.add(operation); if (updateState) publish({ ...state, status: "acting" });
    try { return await act(model, operation.controller.signal); } catch (error) { if (cancelled(error)) return { kind: "cancelled" }; throw error; }
    finally { selecting = false; active.delete(operation); if (updateState && !closed) publish({ ...state, status: "ready" }); operation.signal?.removeEventListener("abort", operation.abort); }
  }
  return Object.freeze({
    ...
    async open({ query, signal } = {}) {
      assertOpen(); if (opened) throw new Error("MODEL_PANEL_ALREADY_OPEN"); opened = true; const normalized = typeof query === "string" && query.trim() !== "" ? query.trim() : undefined; snapshot = runtime.readModelPanelSnapshot(); const cached = panel(snapshot); publish({ panel: cached, query: normalized, refresh: { errors: Object.freeze([]), reason: null, status: "running" }, scope: snapshot.scope, status: "refreshing" });
      if (normalized && snapshot.scope === "scoped") { const exact = findExactModelReferenceMatch(normalized, snapshot.exactCandidates); if (exact) return runSelection(exact, signal); }
      const operation = controller(signal); active.add(operation); let timedOut = false; const timeout = setTimeout(() => { timedOut = true; operation.controller.abort(); if (!normalized && !closed) publish({ ...state, refresh: { errors: Object.freeze([]), reason: "timeout", status: "aborted" }, status: "ready" }); }, refreshTimeoutMs);
      if (!normalized) {
        void runtime.refreshModelPanel({ signal: operation.controller.signal }).then((refreshed) => { if (operation.controller.signal.aborted || closed) return; const view = panel(refreshed); publish({ panel: view, query: undefined, refresh: { errors: Object.freeze([...(refreshed.errors ?? [])]), reason: null, status: "succeeded" }, scope: refreshed.scope, status: "ready" }); }).catch((error) => { if (!cancelled(error) && !closed) publish({ ...state, refresh: { errors: Object.freeze([]), reason: "runtime", status: "failed" }, status: "ready" }); }).finally(() => { clearTimeout(timeout); active.delete(operation); operation.signal?.removeEventListener("abort", operation.abort); });
        return { kind: "panel" };
      }
      try { const refreshed = await runtime.refreshModelPanel({ signal: operation.controller.signal }); if (operation.controller.signal.aborted || closed) return { kind: "cancelled" }; const view = panel(refreshed); publish({ panel: view, query: normalized, refresh: { errors: Object.freeze([...(refreshed.errors ?? [])]), reason: null, status: "succeeded" }, scope: refreshed.scope, status: "ready" }); if (normalized && refreshed.scope === "all") { const exact = findExactModelReferenceMatch(normalized, refreshed.exactCandidates); if (exact) return runSelection(exact, operation.controller.signal); } return { kind: "panel" }; }
      catch (error) { if (!cancelled(error)) throw error; if (!closed) publish({ ...state, refresh: { errors: Object.freeze([]), reason: timedOut ? "timeout" : "cancelled", status: "aborted" }, status: "ready" }); return timedOut ? { kind: "panel" } : { kind: "cancelled" }; }
      finally { clearTimeout(timeout); active.delete(operation); operation.signal?.removeEventListener("abort", operation.abort); }
    },
    async select(ref, { signal } = {}) { assertOpen(); const model = snapshot?.models.find((entry) => entry.provider === ref?.provider && entry.id === ref?.id); if (!model) throw new Error("MODEL_PANEL_SELECTION_STALE"); return runSelection(model, signal, true); },
