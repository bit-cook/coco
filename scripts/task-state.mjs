...
export function validTask(task) {
  ...
  if (!(task.worktree === true || task.worktree === false) || (task.worktreePath !== null && !text(task.worktreePath, 4096)) || (task.branch !== null && !text(task.branch, 256))) return false;
...
export function createTaskStore({ agentDir, now = () => new Date(), random = randomBytes } = {}) {
  ...
  return {
    create,
    load: () => serialized(async () => { await ensureAgentDirectory(directory); return structuredClone(await readState(path)); }),
    path,
    update,
  };
}
