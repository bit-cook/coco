...
export function planModelPanelRollout({ capabilities, enabled = false, evidence, extension, remoteReceipt, scope = "production" } = {}) {
  ...
  if (evidence?.schemaVersion !== 1 || evidence?.status !== "candidate-evidence-only" || evidence?.promotionAuthorized !== false || evidence?.authorization?.isolated !== true || evidence?.authorization?.production !== false || evidence?.authorization?.scope !== "isolated" || evidence?.authorization?.source !== "automated-evidence-gates" || evidence?.gates?.productionRegistration !== "blocked" || evidence?.gates?.fallbackIntegration !== "isolated-loader-pty-verified-not-production") return fallback("MODEL_PANEL_EVIDENCE_INVALID");
