...
export async function verifySelectiveForkEvidence({ evidencePath = join(root, evidenceFile), artifactPath } = {}) {
  ...
  if (evidence.authorization?.isolated !== true || evidence.authorization?.production !== false || evidence.authorization?.scope !== "isolated" || evidence.authorization?.source !== "automated-evidence-gates") return reject("SELECTIVE_FORK_AUTHORIZATION_INVALID");
