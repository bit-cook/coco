...
function budget(value) {
  if (!value || typeof value !== "object" || !Number.isSafeInteger(value.maxChildren) || value.maxChildren < 1 || !Number.isSafeInteger(value.maxCostMicros) || value.maxCostMicros < 0 || !Number.isSafeInteger(value.maxTokens) || value.maxTokens < 1 || !Number.isSafeInteger(value.maxTimeMs) || value.maxTimeMs < 1 || !Number.isSafeInteger(value.maxTurns) || value.maxTurns < 1) fail("ORCH_BUDGET_INVALID");
  return { maxChildren: value.maxChildren, maxCostMicros: value.maxCostMicros, maxTimeMs: value.maxTimeMs, maxTokens: value.maxTokens, maxTurns: value.maxTurns };
}
function cost(value) {
  if (!value || typeof value !== "object" || !Number.isSafeInteger(value.costMicros) || value.costMicros < 0 || !Number.isSafeInteger(value.timeMs) || value.timeMs < 0 || !Number.isSafeInteger(value.tokens) || value.tokens < 0 || !Number.isSafeInteger(value.turns) || value.turns < 0) fail("ORCH_CHILD_COST_INVALID");
  return { costMicros: value.costMicros, timeMs: value.timeMs, tokens: value.tokens, turns: value.turns };
}
function migrate(state) { if (state?.schemaVersion === 1 && Array.isArray(state.parents)) for (const parent of state.parents) { if (!("status" in parent)) parent.status = "active"; if (!("maxCostMicros" in parent.budget)) parent.budget.maxCostMicros = Number.MAX_SAFE_INTEGER; if (Array.isArray(parent.children)) for (const child of parent.children) { if (!("costMicros" in child.cost)) child.cost.costMicros = 0; if (!("actual" in child)) child.actual = null; else if (child.actual && !("costMicros" in child.actual)) child.actual.costMicros = 0; } } return state; }
  ...
  return Object.freeze({
    ...
    async reserve(parentId, childId, childCost) {
      ...
      await applyStateTransaction({ agentDir, operations: async () => {
        ...
        const used = active.reduce((sum, c) => ({ costMicros: sum.costMicros + c.cost.costMicros, timeMs: sum.timeMs + c.cost.timeMs, tokens: sum.tokens + c.cost.tokens, turns: sum.turns + c.cost.turns }), { costMicros: 0, timeMs: 0, tokens: 0, turns: 0 });
        if (used.costMicros + childCost.costMicros > parent.budget.maxCostMicros || used.timeMs + childCost.timeMs > parent.budget.maxTimeMs || used.tokens + childCost.tokens > parent.budget.maxTokens || used.turns + childCost.turns > parent.budget.maxTurns) fail("ORCH_CHILD_BUDGET_EXCEEDED");
