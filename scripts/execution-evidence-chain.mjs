...
export function verifyExecutionEvidenceChain({ preflight, binding, matrixEvidence, descriptor, cases, receipt } = {}) {
  ...
  if (matrix.providerId !== bindingResult.providerId || matrix.providerId !== preflight.providerId) fail("EXECUTION_EVIDENCE_PROVIDER_MISMATCH");
  ...
  return Object.freeze({ matrix: matrix.status, providerId: matrix.providerId, receiptSha256, requestSha256: bindingResult.requestSha256, schemaVersion: 1, status: "verified" });
}
