...
Install the `coco-agent-0.3.13.vsix` asset from the CoCo `v0.3.13` release, start the local service with `coco control start`, and obtain its token with `coco control token`. Verify it first with the release `coco-agent-0.3.13.vsix.sha256` sidecar.
