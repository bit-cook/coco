...
CFG-001 generation evidence is current on `candidate/v0.7.0`: focused generation/state/service tests `14/14`, complete core `633/633`, package closure `175`, scanner clean, package `2/2`, and runtime probe `20,691`. Registry, composition, consumer, service, and restart metadata are implemented. Concrete Pi provider wiring is blocked because the current provider hooks lack a shared request correlation ID; other 0.7.0 items remain pending rather than bypassing this boundary.
