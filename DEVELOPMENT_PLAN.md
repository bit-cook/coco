...
CFG-001 generation evidence is current on `candidate/v0.7.0`: focused provider/MCP/generation tests `20/20`, complete core `630/630`, package closure `175`, scanner clean, package `2/2`, and runtime probe `20,690`. Provider and MCP candidates prepare as one generation; the service exposes reload, rollback, provider/MCP calls, non-secret status, and immutable leases. The next integration persists restart metadata. Other 0.7.0 items remain pending.
