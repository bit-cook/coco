...
if [ -n "$COCO_BIN_DIR" ]; then
  if [ -e "$COCO_BIN_DIR/coco" ] || [ -L "$COCO_BIN_DIR/coco" ]; then
    if [ -w "$COCO_BIN_DIR" ]; then rm -f -- "$COCO_BIN_DIR/coco"
    elif command -v sudo >/dev/null 2>&1; then sudo rm -f -- "$COCO_BIN_DIR/coco"
    else die "Run as root to remove $COCO_BIN_DIR/coco"
    fi
  fi
fi
