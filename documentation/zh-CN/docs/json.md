...
将所有会话事件作为 JSON 行输出到 stdout。适合将 pi 集成到其他工具或自定义 UI 中。
...
`queue_update` 会在待处理的 steering 和 follow-up 队列发生变化时发出完整队列。`compaction_start` 和 `compaction_end` 同时涵盖手动和自动压缩。
