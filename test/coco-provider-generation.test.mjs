...
test("provider lifecycle binds one generation and releases every terminal outcome", async () => {
  ...
  await handlers.get("after_provider_response")({ attempt: 1, headers: {}, requestId: "request-1", status: 200 }, context);
...
test("provider lifecycle aborts missing and duplicate correlation", async () => {
  ...
  await assert.rejects(handlers.get("after_provider_response")({ requestId: "missing" }, context), /PROVIDER_GENERATION_LEASE_MISSING/);
