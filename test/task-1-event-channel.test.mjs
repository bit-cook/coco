...
testWithWritableCgroup("Given caught TCP plus old-path truncation and forged zero output, when guarded, then parent rejects", async () => {
...
testWithWritableCgroup("Given clean direct and descendant children, when channel lifecycle completes, then parent approves zero attempts", async () => {
...
testWithWritableCgroup("Given abrupt exit before guard summary, when channel closes, then it fails closed", async () => {
...
testWithWritableCgroup("Given a forged FD summary before a caught TCP attempt, when frames conflict, then parent rejects", async () => {
...
testWithWritableCgroup("Given a forged FD summary without a network call, when duplicate summary conflicts with guard lifecycle, then it is inconclusive", async () => {
  const { fixture, result } = await run("const s=process.env.COCO_EGRESS_SESSION;require('fs').writeSync(3,JSON.stringify({v:1,session:s,pid:process.pid,seq:2,type:'summary',fields:{attempts:0}})+'\\n')");
