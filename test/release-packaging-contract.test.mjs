...
test("Given public release metadata, when the Pages launcher runs, then it executes the newest stable exact-tag installer and rejects malformed metadata", async () => {
  ...
  try {
    ...
    await mkdir(bin);
    await writeFile(metadata, '[{"tag_name":"v0.1.2","draft":false,"prerelease":false},{"tag_name":"v0.1.3","draft":false,"prerelease":false},{"tag_name":"v0.1.5","draft":false,"prerelease":false},{"tag_name":"v0.1.6","draft":false,"prerelease":false},{"tag_name":"v9.0.0","draft":true,"prerelease":false}]\n');
    await writeFile(installer, '#!/usr/bin/env bash\nprintf "%s\\n" "$COCO_VERSION" > "$COCO_TEST_RESULT"\n');
    await writeFile(join(bin, "curl"), '#!/usr/bin/env bash\nset -euo pipefail\nfor argument in "$@"; do\n  case "$argument" in\n    http*) url="$argument" ;;\n    -o) output=1 ;;\n    *) if [ "${output:-0}" = 1 ]; then target="$argument"; output=0; fi ;;\n  esac\ndone\ncase "$url" in\n  *api.github.com*) cp "$COCO_TEST_METADATA" "$target" ;;\n  */v0.1.6/install.sh) cp "$COCO_TEST_INSTALLER" "$target" ;;\n  *) exit 1 ;;\nesac\n');
    ...
    assert.equal((await readFile(result, "utf8")).trim(), "0.1.6");
...
test("Given the v0.1.6 release contract, when public release surfaces are inspected, then every version and package artifact is consistent", async () => {
  const version = "0.1.6";
