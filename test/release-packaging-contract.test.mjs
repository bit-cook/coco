...
test("Given release workflows, when GitHub Actions and execution controls are configured, then pins are current, CI is cancellable, and jobs are bounded", async () => {
  ...
  assert.match(pagesWorkflow, /runs-on: \[self-hosted, Linux, X64, coco-ci\]/);
...
test("Given the v0.6.1 release contract, when public release surfaces are inspected, then every version and package artifact is consistent", async () => {
  const version = "0.6.1";
