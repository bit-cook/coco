...
const uninstall = "curl -fsSL https://github.com/bit-cook/coco/releases/download/v0.7.4/uninstall.sh | bash";
...
test("bilingual homepage keeps the install-first static site contract", async () => {
  ...
  for (const [page, lang, canonical] of [
    ...
    for (const evidence of ["0.7.4", "676\/676", "39\/39", "9\/9"]) assert.match(page, new RegExp(evidence));
