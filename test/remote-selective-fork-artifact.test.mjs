...
test("promotion verification workflow is manual, read-only, bounded, and isolated on the upstream runner", async () => {
  ...
  assert.match(workflow, /verify-isolated-model-panel-candidate\.mjs/); assert.match(workflow, /scope!=="isolated"/); assert.match(workflow, /loader\?\.fallbackOwner!=="fallback"/); assert.match(workflow, /pty\?\.status!=="passed"/); assert.match(workflow, /promotionAuthorized!==false/); assert.match(workflow, /retention-days: 30/);
