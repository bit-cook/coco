...
test("promotion verification workflow is scheduled and manual, read-only, bounded, and isolated on its dedicated runner", async () => {
  ...
  assert.match(workflow, /verify-model-panel-rollback\.mjs/); assert.match(workflow, /value\.owner!=="fallback"/); assert.match(workflow, /value\.productionRegistration!=="blocked"/);
