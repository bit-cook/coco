...
test("task receipts are canonical, private, and idempotent", async () => {
  ...
  try {
    ...
    const input = { endedAt, exitCode: 0, runId, startedAt, taskId };
    ...
    assert.equal(first.verdict, "passed"); assert.equal(first.log, null);
...
test("task receipts reject conflicting terminal evidence and invalid log references", async () => {
  ...
  try {
    const store = createTaskReceiptStore({ agentDir });
    await store.write({ endedAt, exitCode: 1, runId, startedAt, taskId });
    await assert.rejects(store.write({ endedAt, exitCode: 0, runId, startedAt, taskId }), /TASK_RECEIPT_CONFLICT/);
