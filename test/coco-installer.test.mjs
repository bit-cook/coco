...
async function writeChecksum(tarball) {
  ...
  await writeFile(`${tarball}.sha256`, `${digest}  coco-0.1.0.tgz\n`);
}
...
async function fixture() {
  ...
  const tarball = join(server, "coco-0.1.0.tgz");
  ...
  await writeFile(join(bin, "curl"), "#!/usr/bin/env bash\nset -euo pipefail\nfor ((i = 1; i <= $#; i += 1)); do\n  if [ \"${!i}\" = \"-o\" ]; then\n    next=$((i + 1))\n    case \"${!next}\" in\n      *.sha256) cp \"$COCO_TEST_SIDECAR\" \"${!next}\" ;;\n      *) cp \"$COCO_TEST_TARBALL\" \"${!next}\" ;;\n    esac\n    exit 0\n  fi\ndone\nexit 1\n");
  ...
  return {
    ...
    environment: {
      ...
      COCO_TEST_SIDECAR: join(server, "coco-0.1.0.tgz.sha256"),
...
for (const installer of installers) {
  ...
  test(`Given a mismatched checksum, when ${installer} runs, then it preserves the current installation and configuration`, async () => {
    ...
    try {
      ...
      await writeFile(join(setup.server, "coco-0.1.0.tgz.sha256"), `${"0".repeat(64)}  coco-0.1.0.tgz\n`);
