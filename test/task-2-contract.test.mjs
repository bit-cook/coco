...
test("Given an installed core at 0.82.0, when its package closure is verified, then it rejects with the pinned-core code", async () => {
  ...
  try {
    const piSource = "https://github.com/bit-cook/pi-selective-fork/releases/download/coco-v0.82.1-coco.1/earendil-works-pi-coding-agent-0.82.1.tgz";
    await writeFile(join(fixture, "package.json"), JSON.stringify({ bundledDependencies: ["@earendil-works/pi-coding-agent", "@earendil-works/pi-tui", "@modelcontextprotocol/sdk"], dependencies: { "@earendil-works/pi-coding-agent": piSource, "@earendil-works/pi-tui": "0.82.1", "@modelcontextprotocol/sdk": "1.30.0" }, devDependencies: { npm: "11.18.0" }, packageManager: "npm@11.18.0" }));
    await writeFile(join(fixture, "package-lock.json"), JSON.stringify({ lockfileVersion: 3, packages: { "": { dependencies: { "@earendil-works/pi-coding-agent": piSource, "@earendil-works/pi-tui": "0.82.1", "@modelcontextprotocol/sdk": "1.30.0" }, devDependencies: { npm: "11.18.0" } } } }));
