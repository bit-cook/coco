...
test("provider correlation candidate evidence is pinned and fail-closed", async () => {
  assert.deepEqual(await verifyProviderCorrelationCandidate(), { sourceCommit: "b0cdb8109ed24f8a17cd55a278f672de15e6e21a", sourceTag: "coco-v0.82.1-coco.11", status: "approved" });
});
...
test("installed candidate exposes correlated provider lifecycle types", async () => {
  ...
  assert.equal(manifest.cocoCandidate.sourceTag, "coco-v0.82.1-coco.11");
  ...
  const toolLoop = await readFile(new URL("node_modules/@earendil-works/pi-agent-core/dist/agent-loop.js", root), "utf8");
  assert.match(toolLoop, /const segments = \[\]/); assert.doesNotMatch(toolLoop, /hasSequentialToolCall/);
});
