...
test("only CFG-001 is active and research work does not authorize external runtime adoption", async () => {
  ...
  for (const name of await readdir(directory)) {
    ...
    assert.match(value, name.startsWith("CFG-001") ? /Status: completed/ : name.startsWith("EVID-001") ? /Status: in_progress/ : /Status: pending/); assert.match(value, /Research only|研究|No external code (?:was )?copied/);
