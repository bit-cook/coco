...
test("model panel rollback rejects candidate failures and preserves the official runtime", async () => {
  const calls = [];
  const receipt = await verifyModelPanelRollback({ run: async (command, args, options) => { calls.push({ args, command, hasAgentDir: typeof options.env.COCO_CODING_AGENT_DIR === "string" }); return { stdout: "0.5.3\n" }; } });
  assert.deepEqual(receipt, {
    ...
    officialRuntime: { status: "passed", version: "0.5.3" }, owner: "fallback", productionRegistration: "blocked", schemaVersion: 1, status: "approved",
  });
