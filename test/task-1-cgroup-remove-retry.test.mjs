...
test("Given a clean completion, when egress returns, then its cgroup is immediately absent", async () => {
  let created;
...
test("Given transient remove failures, when egress cleans up, then it retries until the cgroup is absent", async () => {
...
test("Given a persistent remove failure after real removal, when egress returns, then it is typed inconclusive and absent", async () => {
  let created;
...
test("Given a disappearing cgroup remove race, when removal reports ENOENT, then egress completes safely", async () => {
