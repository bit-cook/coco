...
function ownedAppendPrompt(agentDir, appendPath) {
    const ownershipPath = join(agentDir, "ownership.json");
    if (!existsSync(ownershipPath)) {
        return true;
    }
    try {
        const ownership = JSON.parse(readFileSync(ownershipPath, "utf-8"));
        const expected = ownership?.managedFiles?.["APPEND_SYSTEM.md"]?.sourceSha256;
        if (typeof expected !== "string") {
            return true;
        }
        return createHash("sha256").update(readFileSync(appendPath)).digest("hex") === expected;
    }
    catch {
        return false;
    }
...
export class DefaultResourceLoader {
    ...
    discoverAppendSystemPromptFile() {
        ...
        if (existsSync(join(this.agentDir, "SYSTEM.md"))) {
            return undefined;
        }
        if (existsSync(globalPath) && ownedAppendPrompt(this.agentDir, globalPath)) {
            return globalPath;
        }
        return undefined;
    }
